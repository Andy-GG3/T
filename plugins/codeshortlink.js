...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
    ...
    let pler = JSON.parse(fs.readFileSync('./lib/andy/hadiah.json').toString())
  ...
  const prem = JSON.parse(fs.readFileSync("./lib/andy/hadiah.json"))
 // let cpt = `*ADA TRANSAKSI MASUK*\n*DARI @${m.sender.split('@')[0]}*\n\nDENGAN ID DEPOSIT : ${text}\n*Tunggu sampai dia tf hihi*`
  ...
  global.db.data.users[m.sender].exp += 500
   ...
   global.db.data.users[m.sender].lastcode = new Date * 1
     ...
     prem.splice(unp, 1)
fs.writeFileSync("./lib/andy/hadiah.json", JSON.stringify(prem))
        m.reply(capt)
