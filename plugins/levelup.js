...
let handler = async m => {
    ...
    var img = `https://api.lolhuman.xyz/api/rank?apikey=${apichan}&img=${ppUrl}&background=https://telegra.ph/file/2418e7a2c8f4ad25c04d5.jpg&username=${user.name}&level=${user.level}&ranking=${user.role}&currxp=${user.exp}&xpneed=${max - user.exp}`
    let capt = `┌───⊷ LEVEL
▢ Nama : *${user.name}*
▢ Level : *${user.level}*
▢ XP : *(${user.exp - min}/${xp})*
