...
let handler = async (m, { conn, text, args }) => {
  ...
  try {
let res = await fetch(`https://api.lolhuman.xyz/api/pinterest2?apikey=${apichan}&query=${pickRandom(meme)}`)
...
let cita = wes.result[Math.floor(Math.random() * was.length)]
  ...
  conn.sendFile(m.chat, stiker, 'sticker.webp', '', m)
      ...
      let res = await fetch(`https://aemt.me/pinterest?query=${pickRandom(meme)}`)
let wes = await res.json()
                let was = wes.result
let cita = wes.result[Math.floor(Math.random() * was.length)]
   // conn.sendFile(m.chat, cita, null, "*COWOK GANTENG*\n\npencarian dari: cogan " + text, m)
