...
exec(`zip -rv ${archiveName} database lib media mp3 IchanZX plugins schedule scrape src views config.js database.json Credits.txt docker-compose.yml Dockerfile functions.js handler.js index.js install.sh IchanZX.data.json main.js package-lock.json package.json PORT-SC.js Procfile run.js server.js speed.py startup.sh thumbnail.jpg`, (error, stdout, stderr) => {
  if (error) {
